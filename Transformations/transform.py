...
class Rotation(object):
    ...
    def get_angle(self):
        ...
        angle, _ = self.get_angle_and_axis()
        return angle

    def get_axis(self):
        """Return rotation axis as a Vector."""
        _, axis = self.get_angle_and_axis()
        return axis
...
#     """
#
#     def __init__(self, shift=Vector(), rotation=Rotation(), scaling=Scaling(), shift_first=False):
#         """Initialize.
#
#             Parameters
#             ----------
...
#         R = self.rotation
#         s = self.scaling
#         return Transform(-t, R.inverse(), s, shift_first=True)
#
#     ###################################################
#     # BINARY OPERATIONS                               #
#     ###################################################
...
class Transform(object):
    def __init__(self,
                 ...
                 rotation: np.ndarray = np.identity(n=3),
                 scaling: np.ndarray = np.identity(n=3),
    ...
    def inverse(self):
        ...
        t = self.translation
        r = self.rotation
        # m_r = R.from_matrix(r.T)
        # m_r = m_r.as_euler("zyx", degrees=True)
        # m_r = -m_r
        # _, rotation = cv_lib.get_3d_rotation_matrix_from_yaw_pitch_roll(yaw=m_r[0], pitch=m_r[1], roll=m_r[2])
        # r = rotation
        s = self.scaling
        return Transform(-t, np.linalg.inv(r), s, translate_before_rotate=True)
    ...
    def apply_to(self, v, decimals=12):
        """Return transformed Vector."""
        t = self.translation
        if v.shape != t.shape:
            v = v.reshape(t.shape)
    ...
    def scale(self, v):
        ...
        return np.dot(self.scaling, v)

    def rotate(self, v):
        if v.shape[0] != self.rotation.shape[0]:
            v = v.reshape((self.rotation.shape[0], 1))
        euler = self.rotation_as_euler()
        print(euler)
        return np.dot(self.rotation, v)
...
if __name__ == '__main__':
    zero = Vector((0, 0, 0))
    drone_w = Vector((0, 500, 500))
    # ship_w = Vector((500, 500, 0))
    ship_w = Vector((638.257629, 780.390869, 563.966187))
    ship_d = Vector((353.550781, 353.546021, -489.990295))

    roll = Rotation(np.radians(100), 0)  # Setting rotation on x-axis
    pitch = Rotation(np.radians(45), 1)  # Setting rotation on y-axis
    yaw = Rotation(np.radians(75), 2)  # Setting rotation on z-axis
    rot_combined = yaw.before(pitch.before(roll))

    T = Transform(drone_w, rot_combined.inverse())
